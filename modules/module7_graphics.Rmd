...
* Three main frameworks for producing graphics in R: **base**, **`ggplot2`**, and **`lattice`**
* Some consider these to be "basic" and "advanced".
* And some consider them to be quick/dirty versus involved/fancy.
...
c) perhaps better at trellis plots than `ggplot2` (some think this)

d) able to do 3d plots (but be cautious about using 3d plots)
...
For example, here ggplot treats `country` as an aesthetic parameter that differentiates groups of values.
...
**[P](https://www.polleverywhere.com/multiple_choice_polls/tRHisoyMmYVlr6Xv1WD9r)OLL 7A**: Which of these ggplot2 calls will work (in the sense of not giving an error, not in the sense of being a useful plot)?
...
data plotted. `ggplot2` includes a number of helpful default scale functions. For example:
